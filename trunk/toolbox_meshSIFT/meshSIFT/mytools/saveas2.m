...
%Input validation
if nargin < 2 
    error('MATLAB:saveas:tooFewInputs',...
        'Requires handle to Figure or block diagram and filename.' )
end

if ~ishandle(h)
    error('MATLAB:saveas:invalidHandle',...
        'Invalid handle.' )
end
...
if ishghandle(h)
    ...
    if ~strcmp( 'block_diagram', get_param( h, 'type' ) ) ...
        ...
        error('MATLAB:saveas:invalidHandle',...
            'Invalid Simulink handle.' )
    end
end


if ~ischar(name) || isempty(name)
    error('MATLAB:saveas:invalidFilename',...
    'Invalid filename.' );
end
...
if any(name == '*')
    error('MATLAB:saveas:invalidFilename',...
         'Invalid filename: %s, * is not a valid filename character.', name)
end
...
if isempty(fn) 
    error('MATLAB:saveas:invalidFilename',...
        'Invalid filename: %s',name);
end
if ~isempty(fp) && ~exist(fp, 'dir')
    error('MATLAB:saveas:invalidFilename',...
        'Invalid or missing path: %s' ,name  )
end

if nargin == 2
    if isempty(fe) || strcmp(fe,'.')
        error('MATLAB:saveas:badFormat',...
            'No format or filename extension specified: %s', name )
    end
...
if ~isempty(format) && any( exist( ['saveas' format]) == [2 3 5 6] ) %#ok
    ...
    if isempty(i)
        error('MATLAB:saveas:badFormat',...
            'Unsupported format or extension: %s',format)
    else
        print( h, name, ['-d' dev{i} 'cmyk'] )
        return
    end
