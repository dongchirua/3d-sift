function [extr,scales,vertices] = ssextrema(vertex,faces,params)
...
%                    = 5 by default (8 smoothing scales will be calculated)
% params.prior_smoothing = number of smoothing steps before looking for
%                          scale space extrema
%
% OUTPUT
% ------
...
% scales = 1xS array containing the scales of smoothing for the mesh 
% (the scales where scale-space extrema were sought + 3 extra scales)
...
nmax = 0;
nmin = 0;
for ns = 2:numel(scales)-2; % scales where extrema are to be found
    scale = scales(ns); % scale to be assigned to the extrema
    ...
    r = 1;
    for index = maxsc' % loop over maximum candidates
        ...
        if all(dextrvalue(index,ns)>dextrvalue(indices,ns))
            if all(dextrvalue(index,ns)>dextrvalue(indices,ns+1))
                if all(dextrvalue(index,ns)>dextrvalue(indices,ns-1)) % maximum in scale is also maximum wrt neighbours
                    nmax = nmax+1;
                    maxs(nmax,:)=[index scale]; % add tot list of maxima
                end
            end
        end
    end
    for index = minsc' % loop over minimum candidates
        indices = nbhmli(faces,index,r);
        indices = indices(indices~=index);
        if all(dextrvalue(index,ns)<dextrvalue(indices,ns))
            if all(dextrvalue(index,ns)<dextrvalue(indices,ns+1))
                if all(dextrvalue(index,ns)<dextrvalue(indices,ns-1)) % minimum in scale is also minimum wrt neighbours
                    nmin = nmin+1;
                    mins(nmin,:)=[index scale]; % add to list of mimima
                end
            end
        end
    end
