...
%edges_all = unique([face(1:2,:) face(2:3,:) [face(3,:);face(1,:)]]')';
s = [1:m 1:m 1:m];
A = sparse(i,j,s,n,n); 

[i,j,s1] = find(A);     % direct link (D: s1 is triangle number)
[i,j,s2] = find(A');    % reverse link

%I = find( (s1>0) + (s2>0) == 2 );
I = find( ((s1>0) + (s2>0) == 2) & (s1 <= m ) & (s2 <=m) );
% links edge->faces
E = [s1(I) s2(I)];
i = i(I); j = j(I);
% only directed edges
